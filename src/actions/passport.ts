...
export async function generatePassport() {
  ...
  try {
    ...
    const snapshotData = {
      ...
      profile: {
        name: profile?.full_name || profile?.username || "Utkarsh Sinha",
        headline: profile?.headline || "Full-Stack & AI Systems Engineer",
        country: profile?.country || "India",
        college: profile?.college_name || "IIT Delhi",
        avatar_url: profile?.avatar_url || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=400&auto=format&fit=crop&q=80",
        gender: profile?.gender || "Male",
        degree: profile?.degree || "B.Tech – Computer Science Engineering",
      },
      github: {
